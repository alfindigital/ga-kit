...
export function HistoryAnalytics({ history }: HistoryAnalyticsProps) {
  ...
  const urlsPerDay = useMemo(() => {
    ...
    days.forEach(day => {
      countsByDay.set(format(day, 'yyyy-MM-dd'), { utm: 0, qr: 0, yt: 0 });
    });
  ...
  const topSources = useMemo(() => {
    ...
    history
      .filter(item => item.toolType === 'utm' && item.metadata.source)
      .forEach(item => {
  ...
  const topMediums = useMemo(() => {
    ...
    history
      .filter(item => item.toolType === 'utm' && item.metadata.medium)
      .forEach(item => {
    ...
    return (
      <Card>
        <CardContent className="p-6 text-center text-muted-foreground">
          ...
          <p className="text-sm mt-1">Generate some URLs to see usage patterns</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <div className="space-y-4">
        ...
        <Card>
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-xl sm:text-2xl font-bold">
                  {urlsPerDay.reduce((sum, d) => sum + d.UTM + d.QR + d.YT, 0)}
                </p>
      ...
      <Card>
        <CardHeader className="p-3 sm:p-4 pb-2">
          <CardTitle className="text-sm sm:text-base">URLs Generated (Last 14 Days)</CardTitle>
        </CardHeader>
      ...
      {hasUtmData && (
        <div className="grid gap-4 md:grid-cols-2">
          ...
          {topSources.length > 0 && (
            <Card>
              <CardHeader className="p-3 sm:p-4 pb-2">
                <CardTitle className="text-sm sm:text-base">Top UTM Sources</CardTitle>
              </CardHeader>
              <CardContent className="p-3 sm:p-4 pt-0">
          ...
          {topMediums.length > 0 && (
            <Card>
              <CardHeader className="p-3 sm:p-4 pb-2">
                <CardTitle className="text-sm sm:text-base">Top UTM Mediums</CardTitle>
              </CardHeader>
              <CardContent className="p-3 sm:p-4 pt-0">
                <div className="h-[200px]">
                  <ResponsiveContainer width="100%" height="100%">
                ...
                <div key={campaign.name} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="text-xs text-muted-foreground flex-shrink-0">{campaign.value} URLs</span>
                    </div>
                    <div className="h-1.5 bg-muted rounded-full mt-1 overflow-hidden">
                      <div 
                        className="h-full rounded-full transition-all"
                        style={{ 
                          width: `${(campaign.value / topCampaigns[0].value) * 100}%`,
